function [A_hat, loss, loss_true, MSE, grad,kk]=ar1_nuclear_norm(X,X0,init,lambda,epsilon,iters,K)
%estimate sparse A generated by X_t ~ Poisson(exp(nu+A*min(X_{t-1},K))
%using MLE with a nuclear norm regularization penalty
...
while diff>epsilon && kk<iters
    ...
    while ~accept;
        ...
        %A2 is padded with estimate for nu.  Remove this part
        %prior to singular value soft thresholding
        non_nu=A2(2:n,2:n);
        [U,D,V]=svd(non_nu);
